...
define(['ojs/ojcore', 'knockout', 'jquery', 'jet-composites/my-pie/loader', 'jet-composites/my-bar/loader', 'jet-composites/my-line/loader', 'jet-composites/my-divergingstack/loader','jet-composites/my-polarchart/loader'],
        function (oj, ko, $) {
            ...
            function incomeAndExpenditureAnalysisContentViewModel() {
                ...
                self.dataurlarr1.push({dataurl: 'js/data/IncomeAndEx/totalIncome.json', chartname: '总收入分析（月）'});

                self.dataurlarr2 = ko.observableArray();
                self.dataurlarr2.push({dataurl: 'js/data/IncomeAndEx/totalExpend.json', chartname: '总支出分析（月）'});

                self.dataurlarr4 = ko.observableArray();
                self.dataurlarr4.push({dataurl: 'js/data/IncomeAndEx/IncomeAndExTrend.json', chartname: '收入和支出趋势分析'});

                self.dataurlarr5 = ko.observableArray();
                self.dataurlarr5.push({dataurl: 'js/data/IncomeAndEx/DivergingIncomeAndExp.json', chartname: '总收入和支出对比'});

                self.dataurlarr3 = ko.observableArray();
                self.dataurlarr3.push({dataurl: 'js/data/IncomeAndEx/monthylyBill.json', chartname: '按月分析'});

                self.dataurlarr6 = ko.observableArray();
                self.dataurlarr6.push({dataurl: 'js/data/IncomeAndEx/cat_polar.json', chartname: '按小区分析'});
